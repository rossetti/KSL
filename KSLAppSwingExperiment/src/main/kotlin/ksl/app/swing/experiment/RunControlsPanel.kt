    ...
    fun renderUiState(state: UiState) {
        when (state) {
            ...
            is UiState.Running -> {
                ...
                progressBar.isIndeterminate = false
                progressLabel.text =
                    "Point ${state.pointsCompleted} / ${state.totalPoints}"
                val pct = if (state.totalPoints > 0) {
                    (100.0 * state.pointsCompleted / state.totalPoints).toInt()
                } else 0
