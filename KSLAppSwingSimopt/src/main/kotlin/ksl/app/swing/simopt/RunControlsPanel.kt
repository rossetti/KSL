 ...
 *  per solver iteration (`Iteration X / N`).  N is the solver's
 *  `maxIterations` cap — the actual count may be lower if the solver
 *  converges early. */
    ...
    fun renderUiState(state: UiState) {
        when (state) {
            ...
            is UiState.Running -> {
                ...
                progressBar.isIndeterminate = false
                progressLabel.text =
                    "Iteration ${state.iterationsCompleted} / ${state.maxIterations}"
                val pct = if (state.maxIterations > 0) {
                    (100.0 * state.iterationsCompleted / state.maxIterations).toInt()
                } else 0
            ...
            is UiState.Completed -> {
                ...
                progressBar.isIndeterminate = false
                progressLabel.text =
                    "Completed (${state.result.summary.completedItems} iterations)"
