...
fun main(args: Array<String>) {
    ...
    val exercises = mutableListOf<ksl.book.search.BookExercise>()
    for (entry in toc) {
...
private fun writeChunks(
    ...
    exercises: List<ksl.book.search.BookExercise>,
) {
    ...
    File(outDir, "exercises.json").writeText(
        outJson.encodeToString(ListSerializer(ksl.book.search.BookExercise.serializer()), exercises)
    )
}
...
private fun report(chunks: List<BookChunk>, exercises: List<ksl.book.search.BookExercise>) {
    if (chunks.isEmpty()) {
