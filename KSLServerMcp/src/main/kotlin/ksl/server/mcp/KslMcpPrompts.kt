...
object KslMcpPrompts {
    ...
    fun runModelGuidance(bundles: List<BundleInfo>, bundleId: String?, modelId: String?): String {
        ...
        val body = """
            ...
            |4. Full-fidelity, auditable run: if the model ships recipes (`list_recipes`), start from
            |   `get_recipe(name)`; otherwise `run_template(bundleId, modelId)` for a ready-to-edit
    ...
    fun optimizeGuidance(bundles: List<BundleInfo>, bundleId: String?, modelId: String?, objective: String?): String {
        ...
        val body = """
            ...
            |4. Full-fidelity path: if an OPTIMIZATION recipe exists (`list_recipes`), start from `get_recipe(name)`;
            |   edit it, `validate_optimization_config(config)`, then `run_optimization_config(config)`.
    ...
    fun exploreModelGuidance(bundles: List<BundleInfo>, bundleId: String?, modelId: String?): String {
        ...
        val body = """
            ...
            |   - recipes (`list_recipes`): any author-curated starting documents.
            |
