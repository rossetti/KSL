 ...
 * Bundles are discovered from the classpath (the example bundles when run from
 * Gradle) and from the configured bundle directory, which a
 * [BundleDirectoryWatcher] keeps in sync so a JAR dropped there becomes
...
private fun serveStdio() = runBlocking {
    ...
    val watcher = BundleDirectoryWatcher(registry, config.bundlesDir(), config.pollInterval())
    watcher.scanOnce() // pick up already-present jars before serving
