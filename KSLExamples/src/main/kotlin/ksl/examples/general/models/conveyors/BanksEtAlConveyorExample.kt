...
class BanksEtAlConveyorExample(parent: ModelElement, name: String? = null) : ProcessModel(parent, name) {
    ...
    init {
        arrivalConveyor = Conveyor.builder(this, "ArrivalConveyor")
            ...
            .cellSize(10)
            .maxCellsAllowed(2)
            .firstSegment(arrivalArea, myDrillingResource.name, 60)
            .build()
        ...
        exitConveyor = Conveyor.builder(this, "ExitConveyor")
            ...
            .cellSize(10)
            .maxCellsAllowed(2)
            .firstSegment(myInspectionResource.name, exitArea, 100)
            .build()
    }
    ...
    private inner class PartType : Entity() {
        ...
        val productionProcess = process(isDefaultProcess = true) {
            ...
            rideConveyor(cr1, myDrillingResource.name)
            exitConveyor(cr1)
            ...
            while (!done) {
                ...
                if (isType1){
                    val cr2 = requestConveyor(loopConveyor, myDrillingResource.name, 2)
                    rideConveyor(cr2, myMillingResource.name)
                    exitConveyor(cr2)
                    ...
                    val cr3 = requestConveyor(loopConveyor, myMillingResource.name, 2)
                    rideConveyor(cr3, myGrindingResource.name)
                    exitConveyor(cr3)
                } else {
                    val cr4 = requestConveyor(loopConveyor, myDrillingResource.name, 2)
                    rideConveyor(cr4, myPlaningResource.name)
                    exitConveyor(cr4)
                    ...
                    val cr5 = requestConveyor(loopConveyor, myPlaningResource.name, 2)
                    rideConveyor(cr5, myGrindingResource.name)
                    exitConveyor(cr5)
                }
                ...
                val cr6 = requestConveyor(loopConveyor, myGrindingResource.name, 2)
                rideConveyor(cr6, myInspectionResource.name)
                exitConveyor(cr6)
                ...
                if (inspectOutcome == 1.0) {
                    ...
                    // passed, send to exit
                    val cr7 = requestConveyor(exitConveyor, myInspectionResource.name, 2)
                    ...
                    val cr8 = requestConveyor(loopConveyor, myInspectionResource.name, 2)
                    rideConveyor(cr8, myDrillingResource.name)
                    exitConveyor(cr8)
                } else {
    ...
    private inner class PartTypeV2 : Entity() {
        ...
        val productionProcess = process(isDefaultProcess = true) {
            convey(arrivalConveyor, entryLocation = arrivalArea, destination = myDrillingResource.name, numCellsNeeded = 2)
            var done = false
            while (!done) {
                if (myDrillingResource.waitingQ.size >= drillingQCapacity) {
                    myOverflowCounter.increment()
                ...
                if (isType1){
                    convey(loopConveyor, entryLocation = myDrillingResource.name, destination = myMillingResource.name, numCellsNeeded = 2)
                    if (myMillingResource.waitingQ.size >= millingQCapacity) {
                        myOverflowCounter.increment()
                        return@process
                    }
                    use(myMillingResource, delayDuration = myMillingRV)
                    convey(loopConveyor, entryLocation = myMillingResource.name, destination = myGrindingResource.name, numCellsNeeded = 2)
                } else {
                    convey(loopConveyor, entryLocation = myDrillingResource.name, destination = myPlaningResource.name, numCellsNeeded = 2)
                    if (myPlaningResource.waitingQ.size >= planingQCapacity) {
                        myOverflowCounter.increment()
                        return@process
                    }
                    use(myPlaningResource, delayDuration = myPlaningRV)
                    convey(loopConveyor, entryLocation = myPlaningResource.name, destination = myGrindingResource.name, numCellsNeeded = 2)
                }
                ...
                convey(loopConveyor, entryLocation = myGrindingResource.name, destination = myInspectionResource.name, numCellsNeeded = 2)
                ...
                when (myInspectOutcomeRV.value) {
                    1.0 -> {
                        ...
                        // passed, send to exit
                        convey(exitConveyor, entryLocation = myInspectionResource.name, destination = exitArea, numCellsNeeded = 2)
                    ...
                    2.0 -> {
                        ...
                        myReworkCounter.increment()
                        // send back to drilling
                        convey(loopConveyor, entryLocation = myInspectionResource.name, destination = myDrillingResource.name, numCellsNeeded = 2)
                    }
                    else -> {
                        // must be inspectOutcome == 3.0, needs to be scrapped
                        myScrapCounter.increment()
