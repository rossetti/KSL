...
fun main() {
    val m = Model()
    val dtp = MfgSystemActivity(m, name = "MfgSystem")
...
class MfgSystemActivity(
    parent: ModelElement,
    name: String? = null
) : ProcessModel(parent, name) {

    private val timeBetweenArrivalRV: RandomVariable = RandomVariable(parent, ExponentialRV(10.0, 1))
    private val machineSelectionRV: RandomVariable = RandomVariable(
        parent,
        DEmpiricalRV(doubleArrayOf(1.0, 2.0, 3.0), doubleArrayOf(0.2, 0.5, 1.0), 2)
    )
    ...
    private inner class Part : Entity() {
        ...
        val mfgProcess: KSLProcess = process() {
            do {
                val m = machineSelectionRV.value
                if (m == 1.0) {
                    use(machine1, amountNeeded = 1, delayDuration = processingTimeRV)
//                    val m1 = seize(machine1)
//                    delay(processingTimeRV)
//                    release(m1)
                } else if (m == 2.0) {
                    val m2 = seize(machine2)
                    delay(processingTimeRV)
                    release(m2)
                } else {
                    val m3 = seize(machine3)
                    delay(processingTimeRV)
                    release(m3)
                }
                if (needRework == 1.0) {
                ...
                needRework = reworkRV.value
            } while (needRework == 1.0)
        }
    }
