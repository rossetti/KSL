...
/** Named, discoverable [ModelBuilderIfc] for the 'DriveThroughPharmacyWithResource' book example; the build that [BookExamplesBundle] delegates to. */
class DriveThroughPharmacyWithResourceModelBuilder : ModelBuilderIfc {
    override fun build(
        modelConfiguration: Map<String, String>?,
        experimentRunParameters: ExperimentRunParametersIfc?
    ): Model {
        // Child element name ("Pharmacy") must differ from the Model name.
        val model = Model("DriveThroughPharmacyWithResource", autoCSVReports = false)
