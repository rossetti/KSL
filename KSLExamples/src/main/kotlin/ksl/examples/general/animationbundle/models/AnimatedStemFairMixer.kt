...
package ksl.examples.general.animationbundle.models
...
/**
 * The enhanced STEM career-fair mixer with explicit spatial movement, **instrumented for animation**.
 *
 * This is a copy of `ksl.examples.book.chapter8.StemFairMixerEnhancedWithMovement`, not a subclass — the
 * student processes are private, so there is nothing to override. The book examples are deliberately left
 * alone: they teach modelling, and threading animation concerns through them would put a second subject
 * into code written to explain one. Animation gets its own copy to instrument.
 *
 * The model is otherwise the chapter-8 one: NHPP student arrivals, name tags, a conversation area, two
 * recruiter stations and a timed door closing, with the venue laid out as five locations in a
 * [DistancesModel] (Entrance, NameTags, ConversationArea, Recruiting, Exit) that students walk between at
 * a sampled walking speed.
 *
 * **The one difference** is that its two bare delays carry a `suspensionName`. That is what makes them
 * drawable: an animation storage binds to a suspension name, and an unnamed delay is invisible — it is
 * lumped in with every other unnamed delay under the entity type. Naming them turns the two places a
 * student *stops* into elements the layout can place:
 *
 *  - `"NameTags"` — collecting a name tag, half a minute or so.
 *  - `"ConversationArea"` — the conversation itself, averaging about nineteen minutes, and the reason this
 *    model is worth animating: it is where nearly every student in the venue is at any moment.
 *
 * Movement needs no such instrumentation. A `moveTo` already reports the locations it runs between, so the
 * students' paths through the venue come out of the trace on their own.
 *
 * A `suspensionName` is metadata: it names a suspension point and touches neither the random number
 * streams nor the schedule. Adding it leaves every statistic the book model produces unchanged.
 */
class AnimatedStemFairMixer(parent: ModelElement, name: String? = null) : ProcessModel(parent, name) {
    ...
    private inner class Student : Entity() {
        ...
        val mixingStudentProcess = process {
            ...
            if (isClosed) {
                ...
                // get name tags
                delay(myNameTagTimeRV, suspensionName = "NameTags")
                if (isClosed) {
