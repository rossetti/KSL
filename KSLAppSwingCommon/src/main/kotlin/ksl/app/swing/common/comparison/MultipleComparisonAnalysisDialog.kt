...
object MultipleComparisonAnalysisDialog {
    ...
    private class Dialog(
        ...
        private fun parseConfidence(field: JTextField, label: String): Double? {
            ...
            if (v <= 0.0 || v >= 1.0) {
                notifier.warn("$label must lie strictly between 0 and 1; got $v.")
                return null
            }
            return v
        }
        ...
        private fun onGenerate() {
            ...
            val outcome = try {
                ...
                notifier.error("Multiple Comparison render failed: ${t.message ?: t::class.simpleName}")
                return
            }
            outcome.errors.forEach {
                notifier.warn("Multiple Comparison: $it")
            }
            ...
            outcome.htmlPath?.let { html ->
                try {
                    ...
                    notifier.warn(
                        "Multiple Comparison: Browser open: ${t.message ?: t::class.simpleName ?: "unknown error"}"
                    )
                }
            }
        }
