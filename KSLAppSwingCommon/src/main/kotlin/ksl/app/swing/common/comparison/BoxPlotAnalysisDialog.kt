...
/**
 *  Modal configuration dialog for the Box Plot analysis.
 *
 *  Stays open after *Generate* so the analyst can tweak knobs and
 *  re-run.  Close with the *Close* button or the dialog frame's
 *  close gadget.
 *
 *  ## v1 knobs
 *
 ...
 *  into [ComparisonSelectionModel.gatherObservationsFor]'s
 *  LinkedHashMap iteration order and the renderer preserves it.
 */
object BoxPlotAnalysisDialog {
    ...
    private class Dialog(
        ...
        private val captionField = JTextField(40)
        private val xAxisField = JTextField(20).apply {
            toolTipText = "Leave blank to use \"Experiment\""
        }
        private val yAxisField = JTextField(20).apply {
            toolTipText = "Leave blank to use the response name"
        }
        ...
        private fun buildHeader(): JComponent = JPanel().apply {
            ...
            val title = JLabel("Configure Box Plot").apply {
                font = font.deriveFont(Font.BOLD, font.size2D + 1f)
                alignmentX = Component.LEFT_ALIGNMENT
            }
            val sub = JLabel(
                "Boxes are ordered left → right by the experiments column's top → bottom order."
            ).apply {
                foreground = Color(0x55, 0x55, 0x55)
                alignmentX = Component.LEFT_ALIGNMENT
            }
        ...
        private fun buildBody(): JComponent = JPanel(GridBagLayout()).apply {
            ...
            // Row 1 — caption
            gbc.gridy = 1; gbc.gridx = 0; gbc.weightx = 0.0; gbc.fill = GridBagConstraints.NONE
            add(JLabel("Caption:"), gbc)
            gbc.gridx = 1; gbc.weightx = 1.0; gbc.fill = GridBagConstraints.HORIZONTAL
            add(captionField, gbc)

            // Row 2 — axis labels
            gbc.gridy = 2; gbc.gridx = 0; gbc.weightx = 0.0; gbc.fill = GridBagConstraints.NONE
            add(JLabel("Axis labels:"), gbc)
            gbc.gridx = 1; gbc.weightx = 1.0; gbc.fill = GridBagConstraints.HORIZONTAL
            add(JPanel().apply {
                layout = BoxLayout(this, BoxLayout.X_AXIS)
                add(JLabel("x:"))
            ...
            // Row 3 — formats
            gbc.gridy = 3; gbc.gridx = 0; gbc.weightx = 0.0; gbc.fill = GridBagConstraints.NONE
            add(JLabel("Formats:"), gbc)
            gbc.gridx = 1; gbc.weightx = 1.0; gbc.fill = GridBagConstraints.HORIZONTAL
            add(JPanel().apply {
                layout = BoxLayout(this, BoxLayout.X_AXIS)
            ...
            // Row 4 — output dir
            gbc.gridy = 4; gbc.gridx = 0; gbc.weightx = 0.0; gbc.fill = GridBagConstraints.NONE
        ...
        private fun onBrowse() {
            val chooser = JFileChooser().apply {
                dialogTitle = "Output directory for Box Plot report"
        ...
        private fun refreshStatus() {
            val v = model.validateForResponse(selectedResponse, AnalysisType.BOX_PLOT)
