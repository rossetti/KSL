 ...
 *  when the developer omits `modelBuilder(...)`.  Drives the controller's bundle-mode launch path.
 *  Shared across apps (it depends only on KSLCore + the common picker panel), so picking a model
 *  looks and feels identical everywhere.
 *
 ...
 *
 *  Because the dialog runs before the app controller exists, it has no parent
...
private class PickerDialog(
    ...
    private fun buildButtonRow(): JPanel = JPanel(BorderLayout()).apply {
        ...
        // "Load JAR…" lives here only at startup; from the in-app "Open Model" it's omitted (it's in the Bundles menu).
        if (showLoadJarButton) add(JPanel(FlowLayout(FlowLayout.LEFT, 0, 0)).apply { add(loadJarButton) }, BorderLayout.WEST)
