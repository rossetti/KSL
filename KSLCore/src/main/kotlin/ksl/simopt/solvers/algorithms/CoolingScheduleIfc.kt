...
abstract class CoolingSchedule(initialTemperature: Double) : CoolingScheduleIfc {
    init {
        require(initialTemperature.isFinite()) {"The initial temperature must be finite."}
...
class LinearCoolingSchedule(
    ...
    init {
        ...
        require(stoppingTemperature < initialTemperature) { "The stopping temperature must be less than the initial temperature" }
    }

    override var initialTemperature: Double = initialTemperature
        set(value) {
            require(value > 0.0) { "The initial temperature must be positive" }
            require(value.isFinite()) {"The initial temperature must be finite."}
