...
class MemorySimulationRunCache private constructor(
    ...
    private val map: MutableMap<ModelInputs, SimulationRun>
    ...
    override val entries: Set<Map.Entry<ModelInputs, SimulationRun>>
        get() = map.entries
    ...
    override fun get(key: ModelInputs): SimulationRun? {
        return map[key]
    }
    ...
    override fun remove(modelInputs: ModelInputs): SimulationRun? {
    ...
    override fun put(modelInputs: ModelInputs, simulationRun: SimulationRun): SimulationRun? {
        ...
        return map.put(modelInputs, simulationRun)
    }
     ...
     *
     * @param modelInputs the RequestData instance containing the model identification, inputs, and response names to validate.
     ...
     */
    fun validatePair(modelInputs: ModelInputs, simulationRun: SimulationRun): Boolean {
        // check if the model identifier matches
        if (modelInputs.modelIdentifier != simulationRun.modelIdentifier) return false
        // first check that the input names are in the simulation run
        for ((inputName, _) in modelInputs.inputs) {
            if (inputName !in simulationRun.inputs.keys) return false
        }
        // now check that the response names are in the simulation run
        for (responseName in modelInputs.responseNames) {
