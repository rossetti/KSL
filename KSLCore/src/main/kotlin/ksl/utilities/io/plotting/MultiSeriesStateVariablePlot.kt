...
/**
 * Plots multiple sample paths (step functions) for a time-weighted response
 * variable on a single set of axes.
 *
 * Each entry in [seriesDataMap] becomes one coloured step-function series.
 * The outer key is the series label shown in the legend (e.g. `"Rep 1"`);
 * the inner map must contain keys `"times"` and `"values"` holding the
 * simulation times and corresponding state values respectively — the same
 * shape returned by [ResponseTrace.traceDataMap] and [ResponseTrace.traceDataMaps].
 *
 ...
 */
class MultiSeriesStateVariablePlot(
     ...
     *
     * Data for each replication in [repNums] is retrieved over the time window
     * [[startTime], [endTime]]. Each series is labelled `"Rep N"` where N is
    ...
    init {
        title  = "Sample paths for $responseName"
        xLabel = "t"
        yLabel = "y(t)"
