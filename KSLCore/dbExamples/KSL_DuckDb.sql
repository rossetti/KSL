-- A database for holding KSL output statistics in a DuckDb database
-- Created 1-10-2025
...
--
-- Note: As of 1-10-2025,
-- DuckDb does not support cascade delete for foreign keys
--    https://duckdb.org/docs/sql/statements/create_table
-- DuckDb does note support ALTER ADD CONSTRAINT.
--    https://duckdb.org/docs/sql/statements/alter_table

CREATE SCHEMA KSL_DB;

-- While close to postgres duckdb does not have generated always identity construct
-- Must make and use a sequence instead for auto-increment primary keys

CREATE SEQUENCE ID_EXPERIMENT START WITH 1 INCREMENT BY 1;

CREATE TABLE KSL_DB.EXPERIMENT
(
    EXP_ID INTEGER DEFAULT nextval('ID_EXPERIMENT') NOT NULL PRIMARY KEY,
...
CREATE UNIQUE INDEX EXP_NAME_UNIQUE ON KSL_DB.EXPERIMENT(EXP_NAME);

CREATE SEQUENCE ID_SIMULATION_RUN START WITH 1 INCREMENT BY 1;
...
(
    RUN_ID               INTEGER DEFAULT nextval('ID_SIMULATION_RUN') NOT NULL PRIMARY KEY,
...
CREATE UNIQUE INDEX SR_NAME_EXP_UNIQUE ON KSL_DB.SIMULATION_RUN(EXP_ID_FK, RUN_NAME);
...
CREATE UNIQUE INDEX ME_NAME_UNIQUE ON KSL_DB.MODEL_ELEMENT(EXP_ID_FK, ELEMENT_NAME);
...
CREATE SEQUENCE ID_CONTROL START WITH 1 INCREMENT BY 1;

-- CONTROL holds the input controls as designated by the user
-- when annotating with KSLControl
CREATE TABLE KSL_DB.CONTROL
(
    CONTROL_ID    INTEGER DEFAULT nextval('ID_CONTROL') NOT NULL PRIMARY KEY,
    EXP_ID_FK     INTEGER          NOT NULL,
...
CREATE SEQUENCE ID_RV_PARAMETER START WITH 1 INCREMENT BY 1;
...
(
    RV_PARAM_ID   INTEGER DEFAULT nextval('ID_RV_PARAMETER') NOT NULL PRIMARY KEY,
    EXP_ID_FK     INTEGER          NOT NULL,
    ELEMENT_ID_FK INTEGER          NOT NULL,
    CLASS_NAME    VARCHAR(510)     NOT NULL,
...
CREATE SEQUENCE ID_WITHIN_REP_STAT START WITH 1 INCREMENT BY 1;
...
(
    ID             INTEGER DEFAULT nextval('ID_WITHIN_REP_STAT') NOT NULL PRIMARY KEY,
    ELEMENT_ID_FK  INTEGER NOT NULL,
    ...
    FOREIGN KEY (SIM_RUN_ID_FK) REFERENCES KSL_DB.SIMULATION_RUN (RUN_ID)
);
...
CREATE UNIQUE INDEX WRS_UNIQUE_ELEMENT_SIMRUN_REPNUM ON KSL_DB.WITHIN_REP_STAT(ELEMENT_ID_FK, SIM_RUN_ID_FK, REP_ID);

CREATE INDEX WRS_ME_FK_INDEX ON KSL_DB.WITHIN_REP_STAT (SIM_RUN_ID_FK, ELEMENT_ID_FK);

CREATE SEQUENCE ID_ACROSS_REP_STAT START WITH 1 INCREMENT BY 1;
...
(
    ID                   INTEGER DEFAULT nextval('ID_ACROSS_REP_STAT') NOT NULL PRIMARY KEY,
    ELEMENT_ID_FK        INTEGER NOT NULL,
    SIM_RUN_ID_FK        INTEGER NOT NULL,
    STAT_NAME            VARCHAR(510),
    STAT_COUNT           DOUBLE PRECISION CHECK (STAT_COUNT >= 0),
...
CREATE SEQUENCE ID_WITHIN_REP_COUNTER_STAT START WITH 1 INCREMENT BY 1;
...
(
    ID            INTEGER DEFAULT nextval('ID_WITHIN_REP_COUNTER_STAT') NOT NULL PRIMARY KEY,
    ELEMENT_ID_FK INTEGER NOT NULL,
    SIM_RUN_ID_FK INTEGER NOT NULL,
    REP_ID      INTEGER NOT NULL CHECK (REP_ID >= 1),
    STAT_NAME     VARCHAR(510),
...
CREATE UNIQUE INDEX WRCS_UNIQUE_ELEMENT_SIMRUN_REPNUM ON KSL_DB.WITHIN_REP_COUNTER_STAT (ELEMENT_ID_FK, SIM_RUN_ID_FK, REP_ID);

CREATE INDEX WRCS_ME_FK_INDEX ON KSL_DB.WITHIN_REP_COUNTER_STAT (SIM_RUN_ID_FK, ELEMENT_ID_FK);

CREATE SEQUENCE ID_BATCH_STAT START WITH 1 INCREMENT BY 1;
...
(
    ID                       INTEGER DEFAULT nextval('ID_BATCH_STAT') NOT NULL PRIMARY KEY,
    ELEMENT_ID_FK            INTEGER NOT NULL,
    SIM_RUN_ID_FK            INTEGER NOT NULL,
    REP_ID                 INTEGER NOT NULL CHECK (REP_ID >= 1),
    STAT_NAME                VARCHAR(510),
...
CREATE SEQUENCE ID_HISTOGRAM START WITH 1 INCREMENT BY 1;

CREATE TABLE KSL_DB.HISTOGRAM
(
    ID                 INTEGER DEFAULT nextval('ID_HISTOGRAM') NOT NULL PRIMARY KEY,
    ELEMENT_ID_FK      INTEGER          NOT NULL,
    SIM_RUN_ID_FK      INTEGER          NOT NULL,
...
CREATE UNIQUE INDEX HIST_UNIQUE_ELEMENT_SIMRUN_BINNUM ON KSL_DB.HISTOGRAM (ELEMENT_ID_FK, SIM_RUN_ID_FK, BIN_NUM);

CREATE SEQUENCE ID_FREQUENCY START WITH 1 INCREMENT BY 1;

CREATE TABLE KSL_DB.FREQUENCY
(
    ID             INTEGER DEFAULT nextval('ID_FREQUENCY') NOT NULL PRIMARY KEY,
    ELEMENT_ID_FK  INTEGER      NOT NULL,
    SIM_RUN_ID_FK  INTEGER      NOT NULL,
...
CREATE UNIQUE INDEX FREQ_UNIQUE_ELEMENT_SIMRUN_VALUE ON KSL_DB.FREQUENCY (ELEMENT_ID_FK, SIM_RUN_ID_FK, VALUE);
