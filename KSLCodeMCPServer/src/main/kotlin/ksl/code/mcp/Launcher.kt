...
/**
 * Single-jar entrypoint for the self-contained KSL Code MCP server — the
 * "idiot-proof" student jar (`java -jar ksl-code-mcp.jar ...`), ported from the
 * KSL Book MCP server's Launcher. Dispatches by mode:
 *
 *   --stdio    run the MCP server over stdin/stdout (what a coding agent launches);
 *   --doctor   self-test: print the version and the bundled index statistics;
 *   --setup    detect installed agents and wire them to this jar (console);
 *   --remove   undo --setup (remove the ksl-code entry from detected agents);
...
fun main(args: Array<String>) {
    ...
    if (System.getProperty("logback.configurationFile") == null) {
        System.setProperty("logback.configurationFile", "logback-ksl-code-mcp.xml")
    }
...
internal fun jarPath(): String =
    ...
    }.getOrDefault("<path-to>/ksl-code-mcp.jar")

// ---- shared report builders (used by the console modes and the GUI) ----

/** In-process self-test: load the bundled index and build the search index once. */
internal fun buildDoctorReport(): String {
    val loadStart = System.currentTimeMillis()
    val store = CodeStore.load()
    val loadMs = System.currentTimeMillis() - loadStart
    val indexStart = System.currentTimeMillis()
    val search = CodeSearch(store)
    val probe = search.search("resource", 1)
    val indexMs = System.currentTimeMillis() - indexStart
    return buildString {
        appendLine("KSL Code MCP server - doctor")
        appendLine("  version:      ${BuildInfo.version}")
        appendLine("  KSL ref:      ${store.meta.kslVersion}  (indexed ${store.meta.buildDate})")
        appendLine("  declarations: ${store.decls.size}")
        ...
        append(
            if (probe.isNotEmpty()) "  OK - the server runs and the KSL source index is bundled."
            else "  WARNING - the index loaded but a probe search returned nothing."
        )
    }
}

internal fun buildSetupReport(jar: String, results: List<SetupResult>): String = buildString {
    appendLine("KSL Code MCP server - setup")
    appendLine()
    if (results.isNotEmpty()) {
        ...
        appendLine()
        appendLine("Restart the agent(s) above, then ask it: \"show me the KSL Resource API\".")
        appendLine()
...
internal fun buildRemoveReport(results: List<SetupResult>): String = buildString {
    appendLine("KSL Code MCP server - remove")
    appendLine()
    if (results.isNotEmpty()) {
        for (r in results) {
            appendLine("  - ${r.agent}: ${r.action}")
...
private fun usage() {
    println("Usage: java -jar ksl-code-mcp.jar [--stdio | --doctor | --setup | --remove | --gui | --version]")
}
