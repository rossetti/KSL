 ...
 * by **merging** a `ksl-code` entry into each agent's own config (preserving
 * everything else — in particular the model server's `ksl` entry and the book
 * server's `ksl-book` entry, so all three coexist), and `--remove`s it cleanly.
 * Any agent not handled here still works via the universal snippet.
 *
 * Ported from the KSL Book MCP server's AgentSetup; the entry key, jar name, and
 * backup suffix are the only intentional differences.
 *
 ...
 * `[mcp_servers.ksl-code]`). The merge/remove functions are pure and unit-tested; a
...
internal object AgentSetup {
    ...
    fun universalSnippet(jarPath: String): String {
        ...
        return "\"ksl-code\": " + prettyJson.encodeToString(JsonObject.serializer(), entry)
    }
}
...
/** Add/replace `mcpServers.ksl-code` in Claude Desktop JSON, preserving the rest. Throws on unparseable input. */
internal fun mergeClaudeJson(existing: String?, command: String, jarPath: String): String {
    ...
    servers["ksl-code"] = buildJsonObject {
        put("command", command)
        putJsonArray("args") { add("-jar"); add(jarPath); add("--stdio") }
    }
...
/** Remove `mcpServers.ksl-code` from Claude Desktop JSON; null if there's nothing to remove. */
internal fun removeClaudeJson(existing: String?): String? {
    ...
    if ("ksl-code" !in servers) return null
    val newServers = servers.toMutableMap().apply { remove("ksl-code") }
    val merged = root.toMutableMap().apply { this["mcpServers"] = JsonObject(newServers) }
    return prettyJson.encodeToString(JsonObject.serializer(), JsonObject(merged))
}

/** Add/replace `[mcp_servers.ksl-code]` in Codex TOML, preserving the rest. Literal-string paths (no escaping). */
internal fun mergeCodexToml(existing: String?, command: String, jarPath: String): String {
    val block = listOf(
        "[mcp_servers.ksl-code]",
        "command = '$command'",
        "args = ['-jar', '$jarPath', '--stdio']",
    )
    val text = existing ?: ""
    if (!Regex("(?m)^\\s*\\[mcp_servers\\.ksl-code]").containsMatchIn(text)) {
        return if (text.isBlank()) block.joinToString("\n") + "\n"
        else text.trimEnd() + "\n\n" + block.joinToString("\n") + "\n"
    }
    val (start, end) = kslCodeTomlTableRange(text) ?: return text
    val lines = text.lines()
    return (lines.subList(0, start) + block + lines.subList(end, lines.size)).joinToString("\n")
}

/** Remove the `[mcp_servers.ksl-code]` table from Codex TOML; null if absent. */
internal fun removeCodexToml(existing: String?): String? {
    val text = existing ?: return null
    val (start, end) = kslCodeTomlTableRange(text) ?: return null
    val lines = text.lines()
...
/** Line range [start, end) of the `[mcp_servers.ksl-code]` table, or null if absent. */
private fun kslCodeTomlTableRange(text: String): Pair<Int, Int>? {
    if (!Regex("(?m)^\\s*\\[mcp_servers\\.ksl-code]").containsMatchIn(text)) return null
    val lines = text.lines()
    val start = lines.indexOfFirst { it.trim() == "[mcp_servers.ksl-code]" }
...
private interface AgentAdapter {
    ...
    /** Remove the ksl-code entry when present; reports "no entry" when there's nothing to do. */
    fun remove(): SetupResult? {
        ...
        return try {
            val updated = removeEntry(f.readText())
                ?: return SetupResult(name, "no KSL code entry found (nothing to remove)", f.path)
            backupOnce(f)
            f.writeText(updated)
...
private fun backupOnce(f: File) {
    val bak = File("${f.path}.ksl-code-backup")
