...
/**
 * Exercises the registry against a real bundle's output. The MM1 manifest bundle
 * is assembled from KSLTestModels' `MM1ModelBuilder` and loaded via
 * `fromDirectories`, serving its descriptor — including the author-nominated
 * catalog the model declares via `curateCatalog`.
 */
class BundleRegistryTest {
    ...
    fun `serves the MM1 descriptor with its nominated catalog`() {
        TestBundles.registry().use { registry ->
            ...
            // MM1 curates a catalog (3 inputs, 3 outputs) — the raw
