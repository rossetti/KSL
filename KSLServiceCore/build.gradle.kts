...
dependencies {
    ...
    // REST (Ktor) and MCP transports embed. It depends on KSLCore only — no
    // transport dependency ever leaks in. kotlinx-datetime and
    // kotlinx-serialization-json arrive transitively via KSLCore's `api`
    // configuration, so the wire DTOs can be @Serializable.
    api(project(":KSLCore"))
