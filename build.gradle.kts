...
// looks at this file.
/*
 * The published gallery is generated by two tasks in two modules: KSLExamples writes the pages, traces and
 * layouts; KSLAppSwingAnimation renders the poster images, because that module owns the canvas and
 * KSLExamples must not depend on the desktop app.
 *
 * Composed here rather than by one module reaching for the other's task, so the two stay unaware of each
 * other and `buildAnimationSite -Pout=...` remains the single command that regenerates everything. A
 * second command that must be remembered is a second command that will be forgotten, and stale posters
