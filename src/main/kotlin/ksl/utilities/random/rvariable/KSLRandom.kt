...
//TODO replace jsl imports with KSL alternatives
...
import jsl.utilities.distributions.NegativeBinomial
import jsl.utilities.distributions.Normal
import jsl.utilities.distributions.Poisson
...
fun rJohnsonB(
    ...
    val y = Math.exp((z - alpha1) / alpha2)
...
private fun rInvGamma(shape: Double, scale: Double, rng: RNStreamIfc): Double {
    ...
    val chi2 = Gamma.invChiSquareDistribution(
        p, v, g,
        Gamma.DEFAULT_MAX_ITERATIONS, JSLMath.getDefaultNumericalPrecision()
    )
    /* ...transfer chi-square to gamma. */x = scale * chi2 / 2.0
    return (x)
}
 ...
 *
 * @param x          the array
 * @param sampleSize the generation size
 * @param streamNum  the stream number from the stream provider to use
 */
