...
/**
 * The pure-JDK process side of server management: discover, health-check, start, stop, and clean up
 * the KSL MCP suite and its related JVMs. `ProcessHandle` + `java.net.http` only — no Swing, no engine
 * — so it is unit-testable and shared by the launcher and any front-end. Re-homed from the Swing
 * seed's `SuiteProcessManager` into the GUI-agnostic management layer.
 */
object ServerProcessInventory {
    ...
    /** Classify a command line by its most specific KSL marker, or null if it is not a KSL JVM. */
    fun classify(command: String): Kind? = markers.firstOrNull { command.contains(it.first) }?.second
    ...
    /** Orphaned (client-less) KSL JVMs — the leaked processes the stdio-exit fix prevents going forward. */
